...
# no longer on disk. Specifically, doing a `rm /etc/caplan/navidrome.toml` 
...
if [ ! -f "$postinstall_flag" ]; then
    ...
    navidrome service install --user navidrome --working-directory /var/lib/navidrome --configfile /etc/caplan/navidrome.toml || :
    ...
    navidrome service stop --configfile /etc/caplan/navidrome.toml && navidrome service start --configfile /etc/caplan/navidrome.toml
fi
